...
class Pagination
{
    ...
    private function getPrePage()
    {
        ...
        if ($this->getCurrentPage() === 1) {
            ...
            return '<li><a href="' . $this->config['url'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() - 1) . $this->config['param'] . ' " >Pre</a></li>';
        }
    }
    ...
    private function getNextPage()
    {
        ...
        if ($this->getCurrentPage() >= $this->gettotalPage()) {
            ...
            // còn không thì trả về HTML code
            return '<li><a href="' . $this->config['url'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() + 1) . $this->config['param'] . '" >Next</a></li>';
        }
    }
    ...
    public function getPagination()
    {
        ...
        if (isset($this->config['full']) && $this->config['full'] === false) {
            ...
            for ($i = ($this->getCurrentPage() - 3) > 0 ? ($this->getCurrentPage() - 3) : 1; $i <= (($this->getCurrentPage() + 3) > $this->gettotalPage() ? $this->gettotalPage() : ($this->getCurrentPage() + 3)); $i++) {
                if ($i === $this->getCurrentPage()) {
                    $data .= '<li class="active" ><a href="'.$this->config['url']. $this->config['param'] .'" >' . $i . '</a></li>';
                } else {
                    $data .= '<li><a href="' . $this->config['url'] . '?' . $this->config['querystring'] . '=' . $i . $this->config['param'] . '" >' . $i . '</a></li>';
                }
            }
        ...
        return '<ul class="pagination pagination-sm no-margin pull-right">' . $this->getPrePage() . $data . $this->getNextPage() . '</ul>';
    }
