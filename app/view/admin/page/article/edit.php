                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="content-image-upload" style="overflow:hidden ;width: 70px ; height: 70px;border-radius: 5px ; border: 1px solid; margin-top: 10px;">
                                <img id="image-upload" src="<?= $article['image'] ?? './public/admin/img/images.png'?>" width="100%" height="100%"  alt="">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Mô tả ngắn :</label>
                            <textarea class="form-control" name="summary" id="summary" rows="6"> <?= $old['summary'] ?? $article['summary']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Mô tả chi tiết :</label>
                            <textarea class="form-control" name="details_description" id="details_description"><?= $old['details_description'] ?? $article['details_description'] ?>></textarea>
                        </div>
                        <div>
                            <label>
                                <input type="checkbox" name="is_hot"
                                       value="1" <?= $article['is_hot'] ? 'checked' : ''?> <?= isset($old['is_hot']) ? 'checked' : ''?>> Nổi bật
                            </label>
                        </div>
                        <div>
                            <label>
                                <input type="checkbox" name="active"
                                       value="1" <?= $article['active'] ? 'checked' : ''?><?= isset($old['active']) ? 'checked' : ''?>> Hiển thị
                            </label>
                        </div>
                    </div>
